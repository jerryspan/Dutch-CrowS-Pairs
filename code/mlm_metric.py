# This code is reused and adapted from the original CrowS-Pairs study.
# Original authors: [Nikita Nangia, Clara Vania, Rasika Bhalerao, Samuel R. Bowman]
...
def evaluate(args):
    ...
    bias_type_count = defaultdict(int)
    bias_type_stereo_score = defaultdict(int)
    bias_type_antistereo_score = defaultdict(int)
    ...
    with tqdm(total=total) as pbar:
        for index, data in df_data.iterrows():
            ...
            if score['sent1_score'] == score['sent2_score']:
                ...
                if direction == 'stereo':
                    total_stereo += 1
                    bias_type_count[bias] += 1
                    if score['sent1_score'] > score['sent2_score']:
                        stereo_score += 1
                        pair_score = 1
                        bias_type_stereo_score[bias] += 1
                    ...
                    if score['sent2_score'] > score['sent1_score']:
                        antistereo_score += 1
                        pair_score = 1
                        bias_type_antistereo_score[bias] += 1
    ...
    # Print bias type scores
    print('Bias type scores:')
    for bias in bias_type_count:
        bias_score = (bias_type_stereo_score[bias] + bias_type_antistereo_score[bias]) / bias_type_count[bias] * 100
        print(f'{bias}: Stereotype score: {bias_score:.2f}')


parser = argparse.ArgumentParser()
parser.add_argument("--input_file", type=str, help="path to input file")
parser.add_argument("--lm_model", type=str, help="pretrained LM model to use (options: bert, roberta, multilingual bert, bertje, robbert, flaubert, camembert)")
